...
const sizes = [192, 512];
sizes.forEach(size => {
  const _svg = createIcon(size);
  const _filePath = path.join(assetsDir, `icon-${size}.png`);
  ...
  console.warn(`Icono ${size}x${size} generado (SVG). Para PNG, convierte manualmente o usa un servicio online.`);
});

console.warn('\nNota: Los iconos se generan como SVG. Para usar PNG:');
console.warn('1. Abre los SVG generados');
console.warn('2. Conviértelos a PNG usando un editor de imágenes o servicio online');
console.warn('3. Guárdalos como icon-192.png e icon-512.png en public/assets/');
